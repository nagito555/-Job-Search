...
#То, как будет отображаться резюме. 
...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_text(
        ...
        "/search_region - позволит вам найти регион по названию\n"
...
async def list_areas(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # Ограничим количество символов в сообщении , по этому разбиваем на части
    max_len = 4000
...
# Поиск по ключевым словам или по целому слову.
async def search_region(update: Update, context: ContextTypes.DEFAULT_TYPE):
    if not context.args:
        await update.message.reply_text("Пожалуйста, укажите часть названия региона. Пример: /search_region Москва")
        return
    ...
    def search_areas(areas_list):
        for area in areas_list:
            if search_text in area['name'].lower():
                results.append(f"{area['name']} - {area['id']}")
            if area.get('areas'):
                search_areas(area['areas'])

    search_areas(data)

    if not results:
        await update.message.reply_text(f"Регион с названием, содержащим '{search_text}', не найден.")
        return

    text = f"*Результаты поиска регионов по '{search_text}':*\n" + "\n".join(results[:30])
    if len(results) > 30:
        text += f"\n\nПоказаны первые 30 результатов из {len(results)}."
...
if __name__ == '__main__':
    ...
    application.add_handler(CommandHandler("search_region", search_region))
